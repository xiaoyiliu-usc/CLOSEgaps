...
def add_gapfilled_reactions(gem_file, universe, paras):
    ...
    max_growth = model.slim_optimize()
    if max_growth == 0.0:
        print("model %s cannot grow in current medium. \n break" % (gem_file.rstrip('.xml')))
    ...
    if add_random_rxns:
        ...
        if int(paras['USE_GENE']):
            gene_df = pd.read_csv(paras['USE_GENE_file'], index_col=0)
            df_merged = pd.merge(df_gapfill, gene_df, left_index=True, right_index=True)
            df_merged.loc[df_merged['gene_id'].notna(), '0'] += 0.1
            df_gapfill = df_merged.iloc[:,:1]
    ...
    cobra.io.write_sbml_model(model_w_gapfill, f'{paras["OUTPUT_DIRECTORY"]}/{gem_file}' + '.xml')
