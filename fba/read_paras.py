...
def read(input_file, gem_name, num_add):
    df = pd.read_csv(input_file, header=None)
    df.columns = ["Field", "Value"]
    df.iloc[4, 1] = num_add
    df.iloc[3, 1] += f'_{df[df["Field"] == "MODEL"].iloc[0, -1]}/{gem_name}'
    df.iloc[10, 1] += f'_{df[df["Field"] == "MODEL"].iloc[0, -1]}_{df[df["Field"] == "NUM_GAPFILLED_RXNS_TO_ADD"].iloc[0, -1]}/{gem_name}'


    if not os.path.exists(df.iloc[10, 1]):
        os.makedirs(df.iloc[10, 1], exist_ok=True)
    ...
    filenames_in_GEM_DIRECTORY = [f[:-4] for f in os.listdir(paras['GEM_DIRECTORY']) if f.endswith('.xml')]
